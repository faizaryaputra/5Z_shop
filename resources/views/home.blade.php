<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Portfolio</title>
    <link href="https://cdn.jsdelivr.net/npm/remixicon@2.5.0/fonts/remixicon.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/remixicon/fonts/remixicon.css" rel="stylesheet">
    <link href="https://unpkg.com/remixicon/fonts/remixicon.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/remixicon@3.5.0/fonts/remixicon.css" rel="stylesheet">
    <style>
/* ======================================================
                    VARIABLES & GENERAL CSS
========================================================*/
* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
  text-decoration: none;
  list-style: none;
  resize: none;
  outline: none;
  border: none;
}

:root {
  --color-bg: #e1e1e1;
  --color-text: #111111;
  --color-primary: linear-gradient(45deg, #ff6600, #00c8ff); /* Gradient Orange -> Light Blue */
  --color-secondary: linear-gradient(45deg, #e65c00, #0099cc); 
  --color-tertiary:rgb(0, 200, 255);
  --color-bg-alt: #f1f1f1;
  --color-user: #ff6600;
  --color-admin: #0099cc;

  --shadow: 6px 6px 12px #bababa, -6px -6px 12px #ffffff;
  --inner-shadow: inset 6px 6px 12px #bababa, inset -6px -6px 12px #ffffff;

  --width-lg: 80%;
  --width-sm: 95%;
  --transition: all 0.4s ease-in;
}

body {
  font-family: "Great Vibes", cursive;
  font-family: "Rubik", sans-serif;
  background: var(--color-bg);
  color: var(--color-text);
  line-height: 1.7;
  height: 100vh;
  width: 100%;
  position: relative;
  overflow-x: hidden;
    overflow-y: scroll; /* Mengizinkan scroll secara vertikal */
}
body.active {
  --color-bg: #212121;
  --color-text: #e0e0e0;
  --color-tertiary:rgb(255, 123, 0);

  --shadow: 0.5rem 0.5rem 1rem #111, -6px -6px 12px #333;
  --inner-shadow: inset 0.5rem 0.5rem 1rem #111, inset -6px -6px 12px #333;
}
::-webkit-scrollbar {
  width: 0.5rem;
}
::-webkit-scrollbar-track {
  box-shadow: var(--inner-shadow);
}
::-webkit-scrollbar-thumb {
  box-shadow: var(--shadow);
  background: var(--color-tertiary);
}
img {
  width: 100%;
  display: block;
  object-fit: cover;
}
a {
  color: var(--color-text);
  font-size: 1rem;
}
span,
p {
  font-size: 1rem;
}
h1,
h2,
h3,
h4 {
  line-height: 1.2;
}
h1 {
  font-size: 2.8rem;
}
h2 {
  font-size: 2.1rem;
}
h3 {
  font-size: 2rem;
}
h4 {
  font-size: 1.1rem;
}
.container {
  margin: 0 auto;
  width: var(--width-lg);
}
section {
  padding: 5rem 0 3rem;
  min-height: 100vh;
}
.title {
  text-align: center;
  margin-bottom: 6rem;
}
.title h2 {
  font-size: 2.5rem;
  display: inline-block;
  color: var(--color-text);
  font-weight: 300;
}
.title h2 span {
  font-size: 2.5rem;
  color: var(--color-tertiary);
  font-weight: 900;
}
.btn {
  padding: 1rem 3rem;
  position: relative;
  cursor: pointer;
  overflow: hidden;
  background: transparent;
  color: var(--color-text);
  font-weight: 700;
  border-radius: 0.5rem;
  box-shadow: var(--shadow);
  transition: all 0.5s ease-in-out;
  -webkit-transition: all 0.5s ease-in-out;
  -moz-transition: all 0.5s ease-in-out;
  -ms-transition: all 0.5s ease-in-out;
  -o-transition: all 0.5s ease-in-out;
  -webkit-border-radius: 0.5rem;
  -moz-border-radius: 0.5rem;
  -ms-border-radius: 0.5rem;
  -o-border-radius: 0.5rem;
}
.btn span {
  z-index: 1;
  font-weight: 800;
  letter-spacing: 0.1rem;
}
.btn.overlay::after {
  content: "";
  position: absolute;
  top: 0;
  left: 0;
  background: var(--color-text);
  height: 10px;
  width: 10px;
  z-index: -1;
  border-radius: 0.5rem;
  -webkit-border-radius: 0.5rem;
  -moz-border-radius: 0.5rem;
  -ms-border-radius: 0.5rem;
  -o-border-radius: 0.5rem;
  transition: all 0.6s ease-in-out;
  -webkit-transition: all 0.6s ease-in-out;
  -moz-transition: all 0.6s ease-in-out;
  -ms-transition: all 0.6s ease-in-out;
  -o-transition: all 0.6s ease-in-out;
  visibility: hidden;
}
.btn:hover {
  color: var(--color-tertiary);
  box-shadow: 1px 1px 10px var(--color-tertiary);
}
.btn.overlay:hover::after {
  visibility: visible;
  transform: scale(100);
  -webkit-transform: scale(100);
  -moz-transform: scale(100);
  -ms-transform: scale(100);
  -o-transform: scale(100);
}
.scrollbar::-webkit-scrollbar {
  height: 0.66rem;
}
.scrollbar::-webkit-scrollbar-track {
  border-radius: 5px;
  -webkit-border-radius: 5px;
  -moz-border-radius: 5px;
  -ms-border-radius: 5px;
  -o-border-radius: 5px;
}
.scrollbar::-webkit-scrollbar-thumb {
  border-radius: 0.5rem;
  -webkit-border-radius: 0.5rem;
  -moz-border-radius: 0.5rem;
  -ms-border-radius: 0.5rem;
  -o-border-radius: 0.5rem;
}
.scrollbar::-webkit-scrollbar-button {
  width: 0.8rem;
}

/* ======================================================
              HOME
========================================================*/
.home {
  height: 100vh;
  background: url('/images/caffeshop.jpg') no-repeat center center/cover;
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  text-align: center;
  color: white;
}

.home-container {
  display: flex; /* Menggunakan flexbox */
  flex-direction: column; /* Semua elemen tersusun vertikal */
  justify-content: center; /* Pusatkan secara vertikal */
  align-items: center; /* Pusatkan secara horizontal */
  text-align: center;
  width: 100%;
}

.home .center .logo {
  max-width: 250px; /* Perbesar ukuran agar lebih proporsional */
  height: auto; /* Biarkan tinggi menyesuaikan */
  display: block;
  margin: 0 auto; /* Pusatkan logo */
  object-fit: contain; /* Pastikan gambar tidak terdistorsi */
}

.home .left {
  margin-top: 1rem;
  text-align: center;
}

.home .center h3 span {
  font-size: 6rem; /* Ukuran lebih besar */
  color: var(--color-tertiary);
}

.multiple-text {
  font-size: 3rem;
  font-weight: bold;
  color: var(--color-tertiary);
  font-family: 'Montserrat', sans-serif;
  letter-spacing: 3px;
  text-shadow: 0px 0px 15px rgba(255, 255, 255, 0.8);
  display: block;
}

.home .left p {
  padding: 1.6rem 0;
}

.home .left .btn {
  padding: 1rem 3rem;
}

.home .social-icons {
  display: flex;
  justify-content: center; /* Pusatkan ikon */
  gap: 1rem;
  margin-top: 35px;
  margin-bottom: 40px;
}

.home .social-icons i {
  height: 5rem;
  width: 5rem;
  box-shadow: var(--shadow);
  background: var(--color-bg);
  padding: 0.8rem;
  font-weight: 800;
  border-radius: 0.2rem;
  transition: all 0.3s ease;
}

.home .social-icons i:hover {
  color: var(--color-tertiary);
  box-shadow: var(--inner-shadow);
}

.about-container {
  display: grid;
  margin-top: 150px;
  grid-template-columns: 1fr 1fr;
  gap: 2rem;
}
.about .left img {
  max-height: 30rem;
  width: auto;
  box-shadow: var(--shadow);
  border-radius: 0.5rem 50% 0.5rem 0.5rem;
  -webkit-border-radius: 0.5rem 50% 0.5rem 0.5rem;
  -moz-border-radius: 0.5rem 50% 0.5rem 0.5rem;
  -ms-border-radius: 0.5rem 50% 0.5rem 0.5rem;
  -o-border-radius: 0.5rem 50% 0.5rem 0.5rem;
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
}
.about .left img:hover {
  border-radius: 50% 0.5rem 0.5rem 0.5rem;
  -webkit-border-radius: 50% 0.5rem 0.5rem 0.5rem;
  -moz-border-radius: 50% 0.5rem 0.5rem 0.5rem;
  -ms-border-radius: 50% 0.5rem 0.5rem 0.5rem;
  -o-border-radius: 50% 0.5rem 0.5rem 0.5rem;
}
.about .right .title {
  text-align: start;
  margin-bottom: 2rem;
}
.about h2 {
  font-weight: 500;
}
.about .right > h3 {
  font-size: 1.3rem;
  color: var(--color-tertiary);
}
.about .right > h3 span {
  font-size: 1.3rem;
}
.about .right > p {
  margin: 0.5rem 0 2rem;
}
.about .right .btn {
  padding: 1rem 2rem;
}
.about .right .container {
  display: flex;
  align-items: start;
  justify-content: center;
  gap: 1rem;
  margin-top: 3rem;
}
.progress-bar {
  position: relative;
}
.about .container .outer-circle {
  height: 160px;
  width: 160px;
  border-radius: 50%;
  -webkit-border-radius: 50%;
  -moz-border-radius: 50%;
  -ms-border-radius: 50%;
  -o-border-radius: 50%;
  padding: 20px;
  box-shadow: var(--shadow);
}
.container .inner-circle {
  height: 120px;
  width: 120px;
  border-radius: 50%;
  -webkit-border-radius: 50%;
  -moz-border-radius: 50%;
  -ms-border-radius: 50%;
  -o-border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  box-shadow: var(--inner-shadow);
}
.progress-bar h4 {
  font-weight: 500;
  font-size: 0.9rem;
}
circle {
  fill: none;
  stroke-width: 20px;
  stroke: url(#GradientColor);
  stroke-dasharray: 450;
  stroke-dashoffset: 450;
  animation: js 2.5s linear forwards;
  -webkit-animation: js 2.5s linear forwards;
}
.container,
.progress-bar:nth-child(2) circle {
  animation: rj 2.5s linear forwards;
  -webkit-animation: rj 2.5s linear forwards;
}
.container,
.progress-bar:nth-child(3) circle {
  animation: nj 2.5s linear forwards;
  -webkit-animation: nj 2.5s linear forwards;
}
@keyframes js {
  100% {
    stroke-dashoffset: 45;
  }
}
@keyframes rj {
  100% {
    stroke-dashoffset: 90;
  }
}
@keyframes nj {
  100% {
    stroke-dashoffset: 145;
  }
}
svg {
  position: absolute;
  top: 0;
  left: 0;
}

.menu-container {
  display: flex;
  flex-wrap: wrap;
  gap: 2rem;
  margin-top: -20px;
}
.menu-container .box {
  width: calc(33.33% - 1.5rem);
  padding: 0 3rem;
  text-align: center;
  background: transparent;
  box-shadow: var(--shadow);
  border-radius: 0.5rem;
  -webkit-border-radius: 0.5rem;
  -moz-border-radius: 0.5rem;
  -ms-border-radius: 0.5rem;
  -o-border-radius: 0.5rem;
  overflow: hidden;
  position: relative;
}
.menu .title h2 {
  font-weight: 500;
  margin-top: 50px;
}
.menu-container .box i {
  font-size: 4rem;
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
  transition-delay: 0.1s;
}
.menu-container .box h4 {
  font-size: 0.9rem;
  font-weight: 500;
  text-transform: uppercase;
  letter-spacing: 0.1rem;
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
  transition-delay: 0.3s;
}
.menu-container .box .btn {
  padding: 0.4rem 2rem;
  margin: 1rem 0 2rem;
  font-weight: 700;
  background: var(--color-text);
  color: var(--color-bg);
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
  transition-delay: 0.5s;
}
.menu-container .box.overlay::after {
  content: "";
  position: absolute;
  left: 0;
  top: 0;
  transform: translate(-450px, -250px);
  -webkit-transform: translate(-450px, -250px);
  -moz-transform: translate(-450px, -250px);
  -ms-transform: translate(-450px, -250px);
  -o-transform: translate(-450px, -250px);
  background-color: var(--color-tertiary);
  border-radius: 0 50% 50% 50%;
  -webkit-border-radius: 0 50% 50% 50%;
  -moz-border-radius: 0 50% 50% 50%;
  -ms-border-radius: 0 50% 50% 50%;
  -o-border-radius: 0 50% 50% 50%;
  height: 33rem;
  width: 31rem;
  z-index: -1;
  transition: all 0.6s ease-in-out;
  -webkit-transition: all 0.6s ease-in-out;
  -moz-transition: all 0.6s ease-in-out;
  -ms-transition: all 0.6s ease-in-out;
  -o-transition: all 0.6s ease-in-out;
}
.menu .box:hover {
  box-shadow: 1px 1px 15px var(--color-tertiary);
  color: var(--color-bg);
}
.menu .box:hover .btn {
  color: var(--color-tertiary);
  box-shadow: none;
  background: var(--color-bg);
}
.menu .box.overlay:hover::after {
  visibility: visible;
  transform: scale(100);
  -webkit-transform: scale(100);
  -moz-transform: scale(100);
  -ms-transform: scale(100);
  -o-transform: scale(100);
}

/* ======================================================
              GALLERY
========================================================*/
.gallery-container {
  display: grid;
  grid-template-columns: 1fr 3fr;
  gap: 2rem;
}
.gallery .title h2 {
  font-weight: 500;
  margin-top: 50px;
}
.gallery-buttons {
  display: flex;
  flex-direction: column;
  gap: 1.5rem;
}
.gallery-tab {
  font-size: 0.9rem;
  padding: 1rem;
  background: none;
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
  text-transform: capitalize;
}
.gallery-tab:hover,
.gallery-tab.active {
  color: var(--color-tertiary);
}
.gallery-container .tab-content {
  margin-top: 30px;
  display: none;
  grid-template-columns: repeat(3, 1fr);
  gap: 0.5rem;
  overflow: hidden;
}
.gallery-container .tab-content .image {
  position: relative;
  overflow: hidden;
  border-radius: 0.5rem;
  -webkit-border-radius: 0.5rem;
  -moz-border-radius: 0.5rem;
  -ms-border-radius: 0.5rem;
  -o-border-radius: 0.5rem;
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
}
.gallery-container .tab-content img {
  box-shadow: var(--shadow);
  border-radius: 0.5rem;
  -webkit-border-radius: 0.5rem;
  -moz-border-radius: 0.5rem;
  -ms-border-radius: 0.5rem;
  -o-border-radius: 0.5rem;
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
}
.gallery-container .tab-content img:hover {
  transform: scale(1.05);
  -webkit-transform: scale(1.05);
  -moz-transform: scale(1.05);
  -ms-transform: scale(1.05);
  -o-transform: scale(1.05);
}
.tab-content.active-content {
  display: grid;
  transition: var(--transition);
  -webkit-transition: var(--transition);
  -moz-transition: var(--transition);
  -ms-transition: var(--transition);
  -o-transition: var(--transition);
  animation: anim 3s linear;
  -webkit-animation: anim 3s linear;
}
@keyframes anim {
  0% {
    transform: translateY(200px);
    -webkit-transform: translateY(200px);
    -moz-transform: translateY(200px);
    -ms-transform: translateY(200px);
    -o-transform: translateY(200px);
    transform: scale(0);
  }
}
.gallery .btn {
  padding: 0.8rem 2.2rem;
}
.gallery .btn.active {
  box-shadow: var(--shadow);
}
.tab-content div img:hover {
  transform: scale(1.1, 1.1);
  -webkit-transform: scale(1.1, 1.1);
  -moz-transform: scale(1.1, 1.1);
  -ms-transform: scale(1.1, 1.1);
  -o-transform: scale(1.1, 1.1);
}
.tab-content:hover img:not(:hover) {
  filter: blur(10px);
  -webkit-filter: blur(10px);
  transform: scale(0.9, 0.9);
  -webkit-transform: scale(0.9, 0.9);
  -moz-transform: scale(0.9, 0.9);
  -ms-transform: scale(0.9, 0.9);
  -o-transform: scale(0.9, 0.9);
}
/* ======================================================
              CONTACT
========================================================*/
.chat-box {
  width: 100%;
  max-width: 700px;
  height: 550px;
  margin: 2rem auto;
  background: var(--color-bg);
  box-shadow: var(--shadow);
  border-radius: 1rem;
  display: flex;
  flex-direction: column;
  overflow: hidden;
  border: 1px solid #e0e0e0;
  position: relative;
}

.chat-messages {
  flex: 1;
  overflow-y: auto;
  padding: 1rem;
  background: var(--color-bg-alt);
  scroll-behavior: smooth;
}

.chat-message {
  margin-bottom: 1rem;
  padding: 0.4rem 0.8rem;
  border-radius: 1.2rem;
  max-width: 75%;
  position: relative;
  line-height: 1.2;
  font-size: 0.9rem;
  word-break: break-word;
  white-space: pre-wrap;
}

.chat-message.user {
  background-color: var(--color-user);
  align-self: flex-end;
  text-align: right;
  border-bottom-right-radius: 0;
}

.chat-message.admin {
  background-color: var(--color-admin);
  align-self: flex-start;
  text-align: left;
  border-bottom-left-radius: 0;
}

.chat-message strong {
  font-weight: 600;
  display: block;
  margin-bottom: 0.2rem;
  font-size: 0.85rem;
}

#chat-form {
  display: flex;
  gap: 0.5rem;
  padding: 1rem;
  border-top: 1px solid #ddd;
  background: var(--color-bg);
  align-items: center;
}

#chat-input {
  flex: 1;
  padding: 0.75rem 1rem;
  border-radius: 2rem;
  border: 1px solid #ccc;
  outline: none;
  font-size: 0.95rem;
  background: #fefefe;
  color: var(--color-text);
  box-shadow: var(--inner-shadow);
  transition: border 0.3s ease;
}

#chat-input:focus {
  border-color: var(--color-primary);
}

.btn.overlay {
  background-color: var(--color-primary);
  color: white;
  border: none;
  padding: 0.7rem 1.2rem;
  border-radius: 2rem;
  cursor: pointer;
  transition: background 0.3s ease;
}

.btn.overlay:hover {
  background-color: var(--color-primary-hover);
}

/* ======================================================
              RESPONSIVE TABLET
========================================================*/
@media (max-width: 1050px) {
  #menu-btn {
    display: block;
  }
  #theme-btn {
    margin-right: -25px;
  }
  .navlist {
    flex-direction: column;
    position: absolute;
    top: 11rem;
    right: 0;
    font-size: 25px;
    width: var(--width-lg);
    box-shadow: var(--shadow);
    background: var(--color-bg);
    min-width: 300px;
    height: 30rem;
    text-align: center;
    border-radius: 0.2rem;
    -webkit-border-radius: 0.2rem;
    -moz-border-radius: 0.2rem;
    -ms-border-radius: 0.2rem;
    -o-border-radius: 0.2rem;
    padding: 2rem 0;
    z-index: 9999999;
    visibility: hidden;
    opacity: 0;
    transition: var(--transition);
    -webkit-transition: var(--transition);
    -moz-transition: var(--transition);
    -ms-transition: var(--transition);
    -o-transition: var(--transition);
  }
  .navlist a {
    font-size: 20px;
  }
  .navlist.active {
    top: 4rem;
    visibility: visible;
    opacity: 1;
  }
  .about-container {
    grid-template-columns: 1fr;
    gap: 2rem;
  }
  .about-container .left {
    margin: auto;
  }
  .about-container .right {
    text-align: center;
  }
  .about-container .right .title {
    text-align: center;
  }
  .about .right .container {
    gap: 2rem;
  }
  .menu-container .box {
    width: calc(50% - 1.5rem);
  }
  .gallery-container .box {
    width: calc(50% - 1.5rem);
  }
  .gallery-container {
    grid-template-columns: 1fr;
  }
  .gallery-buttons {
    display: grid;
    grid-template-columns: repeat(4, 1fr);
    gap: 1rem;
  }
}
/* ============= Ipad Pro 11 ====================== */
@media (max-width: 835px) {
  h1 {
    font-size: 2.5rem;
  }
  h2 {
    font-size: 1.8rem;
  }
  h3 {
    font-size: 1.3rem;
  }
  p {
    font-size: 1rem;
  }
  section {
    padding: 3rem 0;
  }
  #menu-btn {
    margin-right: -50px;
  }
  .logo {
    margin-left: -50px;
  }
  .home-container {
    grid-template-columns: 1fr;
    margin-top: 20px;
    display: flex;
    flex-direction: column-reverse;
  }
  .home .right img {
    max-width: 25rem;
  }
  .home .left {
    text-align: center;
  }
  .about-container {
    grid-template-columns: 1fr;
    gap: 1rem;
  }
  .about .left img {
    max-height: 25rem;
  }

  .menu .title h2 {
    margin-top: 90px;
    margin-bottom: 20px;
  }
  .menu-container .box {
    width: calc(50% - 1rem);
    margin-bottom: 30px;
  }

  .gallery .title h2 {
    margin-top: 90px;
    margin-bottom: 20px;
  }
  .gallery-buttons {
    grid-template-columns: 1fr 1fr;
  }
  .gallery-container .tab-content {
    grid-template-columns: 1fr 1fr;
    gap: 2rem;
    margin-bottom: 20px;
  }
  .gallery-buttons {
    gap: 2.5rem;
  }
  .contact-content .box {
    padding: 1.2rem 2.1rem;
    border-radius: 0.5rem;
    -webkit-border-radius: 0.5rem;
    -moz-border-radius: 0.5rem;
    -ms-border-radius: 0.5rem;
    -o-border-radius: 0.5rem;
    box-shadow: var(--shadow);
    text-align: center;
  }
  .contact-content .row {
    gap: 1rem;
  }
  .contact-content .contact-form {
    padding: 2rem;
    margin: 6rem 0;
    width: 700px;
  }
  .contact .title h2 {
    margin-top: 100px;
    margin-bottom: 50px;
  }
}
/* ========== Ipad Mini =============== */
@media (max-width: 769px) {
  .home .right img {
    max-width: 15rem;
  }
  .about .left img {
    max-height: 15rem;
  }
}

/* ======================================================
              RESPONSIVE MOBILE
========================================================*/
@media (max-width: 429px) {
  .logo {
    margin-left: -30px;
    font-size: 1rem;
  }
  .logo span {
    font-size: 1rem;
  }
  #menu-btn {
    margin-right: -30px;
    font-size: 13px;
  }
  #theme-btn {
    font-size: 13px;
  }
  .home .right img {
    display: block;
    height: 14rem;
    max-width: 100%;
    margin-top: -15px;
  }
  .home .left {
    position: relative;
    margin-top: -20px;
    z-index: 9999;
  }
  .home .social-icons {
    margin-top: 10px;
    margin-bottom: 40px;
  }
  .home .center h1 {
    line-height: 2.2rem;
    margin-bottom: 2px;
  }
  .home .center h1 span {
    font-size: 2.1rem;
  }
  .home .left h3 {
    font-size: 1.2rem;
    line-height: 2rem;
  }
  .home .left p {
    font-size: 0.7rem;
  }

  .home .social-icons i {
    height: 5rem;
    width: 5rem;
    box-shadow: var(--shadow);
    background: var(--color-bg);
    padding: 0.8rem;
    margin: 0.7rem;
    font-weight: 800;
    border-radius: 0.2rem;
    -webkit-border-radius: 0.2rem;
    -moz-border-radius: 0.2rem;
    -ms-border-radius: 0.2rem;
    -o-border-radius: 0.2rem;
  }
  .about .left img {
    height: 25rem;
    margin-top: -0.6rem;
  }
  .about .right .container {
    align-items: center;
    justify-content: center;
    flex-direction: column;
  }
  .menu .title h2 {
    margin-top: 50px;
  }
  .menu-container .box {
    width: calc(100% - 0rem);
    margin-bottom: 1px;
  }
  .gallery .title h2 {
    margin-top: 50px;
  }
  .gallery-container {
    margin-top: -20px;
  }
  .gallery-container .tab-content {
    grid-template-columns: 1fr;
  }
  .contact {
    min-height: 100vh;
  }
  .contact-content .row {
    margin-top: -55px;
    display: flex;
    flex-direction: column;
    justify-content: center;
    gap: 1rem;
  }
  .contact .title h2 {
    margin-top: 50px;
  }
  .contact-content .row {
    margin-top: -100px;
  }

  .contact-content .box {
    width: 400px;
    margin: auto;
    padding: 1rem 1rem;
    border-radius: 0.5rem;
    -webkit-border-radius: 0.5rem;
    -moz-border-radius: 0.5rem;
    -ms-border-radius: 0.5rem;
    -o-border-radius: 0.5rem;
    box-shadow: var(--shadow);
    text-align: center;
  }

  .contact-content .contact-form {
    padding: 2rem;
    margin: 8.5rem auto;
    width: 93%;
    margin-bottom: -10px;
  }
  .contact-form .btn {
    display: flex;
    margin: auto;
    padding: 1rem 1.5rem;
  }
  .contact-form .btn span {
    font-weight: 900;
    font-size: 96%;
    letter-spacing: 0.05rem;
  }
}
    </style>
    <body>
      @include('component.navbar')
    <!-- ==================================
                  HOME SECTION
    ==================================== -->
    <section class="home" id="home">
  <div class="container home-container">
    <div class="center">
    <img src="images/5Zcaffeshop.jpg" alt="Logo" class="logo">
    <h3><div class="multiple-text">Smart Coffee for a Smarter You</div></h3>
      <div class="social-icons-container">
        <div class="social-icons">
          <a href="https://instagram.com" target="_blank"><i class="ri-instagram-line"></i></a>
          <a href="https://whatsapp.com" target="_blank"><i class="ri-whatsapp-line"></i></a>
          <a href="https://github.com" target="_blank"><i class="ri-github-line"></i></a>
          <a href="https://linkedin.com" target="_blank"><i class="ri-linkedin-line"></i></a>
        </div>
        <button class="btn overlay" id="btn">
          <span>View Menu</span>
        </button>
      </div>
    </div>
  </div>
</section>

    <!-- ==================================
                  ABOUT SECTION
    ==================================== -->
    <section class="about" id="about">
      <div class="container about-container">
        <div class="left">
          <!-- add your photo in src for About page from assets/images/ -->
          <img src="assets/images/profile.png" alt="" />
        </div>
        <div class="right">
          <div class="title">
            <h2>Cafe <span>5Z</span></h2>
          </div>
          <h3>
            CAFE 5Z
          </h3>
          <p>
          I am an active Gunadarma University student majoring in Information Systems with a strong passion for UI/UX design, front-end development, and back-end development.
          </p>
          <button class="btn overlay">
            <span>Download CV</span>
          </button>
          <div class="container">
            <div class="progress-bar">
              <div class="outer-circle">
                <div class="inner-circle">
                  <div class="html-number number">
                    <h4>UI/UX Designer</h4>
                  </div>
                </div>
              </div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                version="1.1"
                width="160px"
                height="160px"
              >
                <defs>
                  <linearGradient id="GradientColor">
                    <stop offset="0%" stop-color="rgb(47, 206, 255)" />
                    <stop offset="100%" stop-color="rgb(255, 123, 0)" />
                  </linearGradient>
                </defs>
                <circle cx="80" cy="80" r="70" stroke-linecap="round" />
              </svg>
            </div>
            <div class="progress-bar">
              <div class="outer-circle">
                <div class="inner-circle">
                  <div class="html-number number">
                    <h4>Front-end dev</h4>
                  </div>
                </div>
              </div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                version="1.1"
                width="160px"
                height="160px"
              >
                <defs>
                  <linearGradient id="GradientColor">
                    <stop offset="0%" stop-color="rgb(47, 206, 255)" />
                    <stop offset="100%" stop-color="rgb(255, 123, 0)" />
                  </linearGradient>
                </defs>
                <circle cx="80" cy="80" r="70" stroke-linecap="round" />
              </svg>
            </div>
            <div class="progress-bar">
              <div class="outer-circle">
                <div class="inner-circle">
                  <div class="html-number number">
                    <h4>Back-end dev</h4>
                  </div>
                </div>
              </div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                version="1.1"
                width="160px"
                height="160px"
              >
                <defs>
                  <linearGradient id="GradientColor">
                    <stop offset="0%" stop-color="rgb(47, 206, 255)" />
                    <stop offset="100%" stop-color="rgb(255, 123, 0)" />
                  </linearGradient>
                </defs>
                <circle cx="80" cy="80" r="70" stroke-linecap="round" />
              </svg>
            </div>
          </div>
        </div>
      </div>
    </section>

    <section class="menu" id="menu">
  <div class="title">
    <h2>Our <span>Menu</span></h2>
  </div>
  <div class="container menu-container">
    <div class="box overlay">
      <div class="content">
        <i class="ri-cup-fill"></i>
        <h4>Hot Coffe</h4>
        <form action="{{ route('orders.create') }}" method="GET">
    <input type="hidden" name="menu" value="Hot Coffe"> {{-- atau sesuai nama menu --}}
    <button class="btn" type="submit">Order Now</button>
</form>
      </div>
    </div>
    <div class="box overlay">
      <div class="content">
        <i class="ri-cup-line"></i>
        <h4>Cold Coffe</h4>
        <form action="{{ route('orders.create') }}" method="GET">
    <input type="hidden" name="menu" value="Cold Coffe"> {{-- atau sesuai nama menu --}}
    <button class="btn" type="submit">Order Now</button>
</form>
      </div>
    </div>
    <div class="box overlay">
      <div class="content">
        <i class="ri-cake-2-fill"></i>
        <h4>Dessert</h4>
        <form action="{{ route('orders.create') }}" method="GET">
    <input type="hidden" name="menu" value="Dessert"> {{-- atau sesuai nama menu --}}
    <button class="btn" type="submit">Order Now</button>
</form>
      </div>
    </div>
    <div class="box overlay">
      <div class="content">
        <i class="ri-restaurant-fill"></i>
        <h4>Food</h4>
        <form action="{{ route('orders.create') }}" method="GET">
    <input type="hidden" name="menu" value="Food"> {{-- atau sesuai nama menu --}}
    <button class="btn" type="submit">Order Now</button>
</form>
      </div>
    </div>
    <div class="box overlay">
      <div class="content">
        <i class="ri-bread-fill"></i>
        <h4>Bread & Pastry</h4>
        <form action="{{ route('orders.create') }}" method="GET">
    <input type="hidden" name="menu" value="Bread & Pastry"> {{-- atau sesuai nama menu --}}
    <button class="btn" type="submit">Order Now</button>
</form>
      </div>
    </div>
    <div class="box overlay">
      <div class="content">
        <i class="ri-bubble-chart-fill"></i> <!-- Contoh ikon alternatif -->
        <h4>Other Drink</h4>
        <form action="{{ route('orders.create') }}" method="GET">
    <input type="hidden" name="menu" value="Other Drink"> {{-- atau sesuai nama menu --}}
    <button class="btn" type="submit">Order Now</button>
</form>
      </div>
    </div>
  </div>
</section>

     <!-- ==================================
                gallery SECTION
    ==================================== -->
    <section class="gallery" id="gallery">
        <div class="title">
            <h2>CafeShop <span>Gallery</span></h2>
        </div>
        <div class="container gallery-container">
            <div class="gallery-buttons" >
                <button class="btn gallery-tab active" onclick="tabOpen('all')">
                    All</button>
                <button class="btn gallery-tab " onclick="tabOpen('webdevelop')">
                    Front-end Development</button>
                <button class="btn gallery-tab " onclick="tabOpen('webdesign')">
                    UI/UX Design</button>
                <button class="btn gallery-tab " onclick="tabOpen('appdevelop')">
                    Front-end Development</button>
            </div>
            <div class="tab-content active-content" id="all">
                <!-- add your image or mockup gallery in src for tab-content all  from assets/images/ -->
                <div class="image"><img src="assets/images/img5.png" alt=""></div>
                <div class="image"><img src="assets/images/img2.png" alt=""></div>
                <div class="image"><img src="assets/images/img3.png" alt=""></div>
                <div class="image"><img src="assets/images/img9.png" alt=""></div>
                <div class="image"><img src="assets/images/img12.png" alt=""></div>
                <div class="image"><img src="assets/images/img10.png" alt=""></div>
            </div>
            <div class="tab-content" id="webdevelop">
                <!-- add your image or mockup gallery in src for tab-content webdevelop from assets/images/ -->
                <div class="image"><img src="assets/images/img2.png" alt=""></div>
                <div class="image"><img src="assets/images/img3.png" alt=""></div>
                <div class="image"><img src="assets/images/img4.png" alt=""></div>
                <div class="image"><img src="assets/images/img5.png" alt=""></div>
                <div class="image"><img src="assets/images/img6.png" alt=""></div>
                <div class="image"><img src="assets/images/img7.png" alt=""></div>
            </div>
            <div class="tab-content" id="webdesign">
                <!-- add your image or mockup gallery in src for tab-content webdesign from assets/images -->
                <div class="image"><img src="assets/images/img8.png" alt=""></div>
                <div class="image"><img src="assets/images/img9.png" alt=""></div>
                <div class="image"><img src="assets/images/img10.png" alt=""></div>
                <div class="image"><img src="assets/images/img11.png" alt=""></div>
                <div class="image"><img src="assets/images/img12.png" alt=""></div>
                <div class="image"><img src="assets/images/img13.png" alt=""></div>
            </div>
            <div class="tab-content" id="appdevelop">
                <!-- add your image or mockup gallery in src for tab-content appdevelop from assets/images -->
                <div class="image"><img src="assets/images/portfolio (19).jpg" alt=""></div>
                <div class="image"><img src="assets/images/portfolio (4).jpg" alt=""></div>
                <div class="image"><img src="assets/images/portfolio (14).jpg" alt=""></div>
                <div class="image"><img src="assets/images/portfolio (18).jpg" alt=""></div>
                <div class="image"><img src="assets/images/portfolio (15).jpg" alt=""></div>
                <div class="image"><img src="assets/images/portfolio (2).jpg" alt=""></div>
            </div>
        </div>
    </section>
   <!-- ==================================
                CONTACT SECTION
   ==================================== -->
   <section class="contact" id="contact">
  <div class="title">
    <h2>Live <span>Chat</span></h2>
  </div>
  <div class="contact-content">
    <div class="chat-box">
      
      <!-- Chat Messages -->
      <div id="chat-messages" class="chat-messages">
        <!-- Pesan akan muncul di sini oleh JS -->
      </div>

      <!-- Form Kirim Pesan -->
      <form action="{{ route('chat.send') }}" method="POST" id="chat-form">
        @csrf
        <input type="text" id="chat-input" name="message" placeholder="Ketik pesan..." required>
        <button type="submit" class="btn overlay">
          <span>Kirim</span>
        </button>
      </form>

    </div>
  </div>
</section>

    <!-- ==================================
                   Scroll Reveal
    ==================================== -->
    <script src="https://unpkg.com/scrollreveal"></script>
    <!-- ==================================
                   Typed Js
    ==================================== -->
    <script src="https://unpkg.com/typed.js@2.1.0/dist/typed.umd.js"></script>
    <!-- ==================================
                   custom Js
    ==================================== -->
    <script src="assets/js/script.js"></script>
  </body>

<script>
  function fetchMessages() {
    fetch('/chat/fetch')
      .then(res => res.json())
      .then(data => {
        const messagesDiv = document.getElementById('chat-messages');
        messagesDiv.innerHTML = '';

        data.messages.forEach(msg => {
          const messageDiv = document.createElement('div');
          messageDiv.classList.add('chat-message');

          if (msg.from === 'user') {
            messageDiv.classList.add('user');
          } else {
            messageDiv.classList.add('admin');
          }

          messageDiv.innerHTML = `<strong>${msg.name}</strong><br>${msg.message}`;
          messagesDiv.appendChild(messageDiv);
        });

        // Auto scroll to bottom
        messagesDiv.scrollTop = messagesDiv.scrollHeight;
      });
  }

  // Form submit handler
  document.getElementById('chat-form').addEventListener('submit', function(e) {
    e.preventDefault();

    const messageInput = document.getElementById('chat-input');
    const message = messageInput.value.trim();
    if (!message) return;

    fetch(`{{ route('chat.send') }}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
        },
        body: JSON.stringify({ message })
    }).then(res => res.json())
    .then(data => {
        if (data.status === 'sent') {
            messageInput.value = '';
            fetchMessages(); // atau tampilkan langsung chat baru
        }
    });
});
// ============== SCROLL REVEAL ANIMATION ================
const sr = ScrollReveal({
  distance: "200px",
  duration: 3500,
  delay: 200,
  reset: true,
});

sr.reveal(".home-container h3", {
  opacity: 0,  
  scale: 0.8, // Mulai lebih kecil agar efek lebih smooth
  duration: 2500,  
  easing: "ease-in-out",
  beforeReveal: function (el) {
    el.style.opacity = "0"; // Pastikan tidak terlihat di awal
    el.style.transition = "opacity 2s ease-in-out, box-shadow 1.5s ease-in-out"; 
    el.style.boxShadow = "0px 0px 30px rgba(255, 255, 255, 0.9)"; // Cahaya lebih kuat

    setTimeout(() => {
      el.style.opacity = "1"; // Perlahan muncul
    }, 1200); // Delay kecil agar efek smooth

    setTimeout(() => {
      el.style.boxShadow = "none"; // Hilangkan glow setelah muncul
    }, 1500);
  }
});
sr.reveal(".home-container .logo", {
  origin: "top",
  distance: "100px",
  opacity: 0,
  scale: 0.8,
  rotate: { x: 0, y: 180, z: 0 },
  duration: 2500,
  easing: "ease-out"
});
sr.reveal(".home-container .right", {
  opacity: 0,  
  scale: 0.8, // Mulai lebih kecil agar efek lebih smooth
  duration: 2500,  
  easing: "ease-in-out",
  beforeReveal: function (el) {
    el.style.opacity = "0"; // Pastikan tidak terlihat di awal
    el.style.transition = "opacity 2s ease-in-out, box-shadow 1.5s ease-in-out"; 
    el.style.boxShadow = "0px 0px 30px rgba(255, 255, 255, 0.9)"; // Cahaya lebih kuat

    setTimeout(() => {
      el.style.opacity = "1"; // Perlahan muncul
    }, 1200); // Delay kecil agar efek smooth

    setTimeout(() => {
      el.style.boxShadow = "none"; // Hilangkan glow setelah muncul
    }, 1500);
  }
});
sr.reveal(".social-icons-container", {
  opacity: 0,  
  scale: 0.8, // Mulai lebih kecil agar efek lebih smooth
  duration: 2500,  
  easing: "ease-in-out",
  beforeReveal: function (el) {
    el.style.opacity = "0"; // Pastikan tidak terlihat di awal
    el.style.transition = "opacity 2s ease-in-out, box-shadow 1.5s ease-in-out"; 
    el.style.boxShadow = "0px 0px 30px rgba(255, 255, 255, 0.9)"; // Cahaya lebih kuat

    setTimeout(() => {
      el.style.opacity = "1"; // Perlahan muncul
    }, 1200); // Delay kecil agar efek smooth

    setTimeout(() => {
      el.style.boxShadow = "none"; // Hilangkan glow setelah muncul
    }, 1500);
  }
});
sr.reveal(".about-container .title", { origin: "right" });
sr.reveal(".about-container h3", { origin: "bottom" });
sr.reveal(".about-container p", { origin: "bottom" });
sr.reveal(".about-container .left", { origin: "left" });
sr.reveal(".about-container .right", { origin: "right" });
sr.reveal(".menu .title", { origin: "top" });
sr.reveal(".menu .content-1", { origin: "left" });
sr.reveal(".menu .content-2", { origin: "right" });
sr.reveal(".gallery-container", { origin: "bottom" });
sr.reveal(".gallery .title", { origin: "top" });
sr.reveal(".gallery-buttons", { origin: "left" });
sr.reveal(".contact .title", { origin: "top" });
sr.reveal(".contact .row .box", { origin: "right" });
sr.reveal(".contact .row .contact-form", { origin: "left" });
</script>
</body>
</html>